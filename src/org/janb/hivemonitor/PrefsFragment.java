...
public class PrefsFragment extends PreferenceFragment implements OnPreferenceClickListener{
	...
	EditTextPreference contactPref;
	Preference serverManageBTN, gitHubBTN;
...
public void onCreate(Bundle savedInstanceState) {
	...
	act = getActivity();
	getActivity().getActionBar().setTitle(R.string.settingsL);
	screenSetup();
	}
...
private void screenSetup() {
		...
		serverManageBTN = (Preference)findPreference("serverManageBTN");
		gitHubBTN = (Preference)findPreference("gitHubBTN");
		serverManageBTN.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
		                @Override
		                public boolean onPreferenceClick(Preference arg0) { 
		                	Intent set = new Intent(act, WelcomeActivity.class);
		             	   startActivity(set);
		                    return true;
		                }
		            });
		...
		gitHubBTN.setOnPreferenceClickListener(this);
...
class RegisterService extends AsyncTask<String,String,String>{
		...
		private void RegisterQueryDialog(String msg) {
			Log.i("BeeMonitor", "Server Response: " + msg);
			if (msg.equals("registersuccess")){
...
private String MTMRegDereg(String name, String command) {
			if (command.equals("gcmunregister")){
				...
				if (command.equals("register")){
					url = "https://" + SERVER_URL + "/hivemonitor/c2dm/register.php";
				}
				if (command.equals("unregister")){
					url = "https://" + SERVER_URL + "/hivemonitor/c2dm/unregister.php";
				}
